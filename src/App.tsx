...
export const fetchDogs = async (): Promise<Dog[]> => {
  ...
  try {
    ...
    return data.map(dog => {
      const sanitized: any = { ...dog };
      for (const key in sanitized) {
        if (typeof sanitized[key] === "number") {
          sanitized[key] = String(sanitized[key]);
        }
      }
      if (typeof sanitized.sightings === "string") {
        try { sanitized.sightings = JSON.parse(sanitized.sightings); } catch { sanitized.sightings = []; }
      }
      return sanitized as Dog;
    });
  } catch (error) {
